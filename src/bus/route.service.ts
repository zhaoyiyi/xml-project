...
const URL = `https://xml-project.vercel.app/api/routeList?a=ttc`;
...
export class RouteService {
  ...
  public getRoute(num): Observable {
    return this.query('routeConfig', `r=${num}`).map((routes) => {
      ...
      let stops = this.attrArray(routes, 'stop')
        .map((stop) => {
  ...
  private query(cmd: string, ...options: string[]): Observable<any> {
    return this._http
      .get(`${URL}&command=${cmd}&` + options.join('&'))
