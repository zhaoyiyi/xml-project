...
export class MapService {
    ...
    this._buses = buses.map((bus) => {
      if (bus && bus.id) {
        return addMarker(
          this._map,
          bus,
          ICONSET.bus(google.maps.SymbolPath.FORWARD_CLOSED_ARROW, bus)
        );
      }
    });
  }
  ...
  public drawStops(stops: Array<StopPrediction>, infoContent?: Array<string>) {
    ...
    this._stops = stops.map((stop, index) => {
      const stop = addMarker(
        this._map,
        stop,
        ICONSET.stop(google.maps.SymbolPath.CIRCLE)
      );
      if (infoContent) {
        ...
        stop.marker.addListener('click', () =>
          info.open(this._map, stop.marker)
        );
      }
      return stop;
    });
